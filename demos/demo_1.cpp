...
main(int argc, char *argv[]) {
    ...
    std::filesystem::path outFile = "./calib_10.woff2";
    ...
    if (auto rrr = modifier_1.change_makeMonospaced(600); not rrr) { std::exit(1); }
    ...
    if (not otfccxx::write_bytesToFile(outFile, res3.value()).has_value()) { std::exit(1); }
