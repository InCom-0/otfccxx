...
caryll_Buffer *cff_encodeCffOperator(int32_t val) {
    if (val > 256) { return bufninit(2, val / 256, val % 256); }
    else { return bufninit(1, val); }
}

caryll_Buffer *cff_encodeCffInteger(int32_t val) {
    if (val >= -107 && val <= 107) { return bufninit(1, val + 139); }
    ...
    else if (val >= -32768 && val < 32768) { return bufninit(3, 28, val >> 8, val & 0xff); }
    else {
...
caryll_Buffer *cff_encodeCffFloat(double val) {
    ...
    if (val == 0.0) {
        ...
        for (i = 0; i < strlen((char *)temp);) {
            if (temp[i] == '.') { niblen++, i++; }
            else if (temp[i] >= '0' && temp[i] <= '9') { niblen++, i++; }
            else if (temp[i] == 'e' && temp[i + 1] == '-') { niblen++, i += 2; }
            else if (temp[i] == 'e' && temp[i + 1] == '+') { niblen++, i += 2; }
            else if (temp[i] == '-') { niblen++, i++; }
        }
        ...
        for (i = 0; i < strlen((char *)temp);) {
            if (temp[i] == '.') { array[j++] = 0x0a, i++; }
            else if (temp[i] >= '0' && temp[i] <= '9') { array[j++] = temp[i] - '0', i++; }
            else if (temp[i] == 'e' && temp[i + 1] == '-') { array[j++] = 0x0c, i += 2; }
            else if (temp[i] == 'e' && temp[i + 1] == '+') { array[j++] = 0x0b, i += 2; }
            else if (temp[i] == '-') { array[j++] = 0x0e, i++; }
        }
...
static uint32_t cff_dec_r(const uint8_t *start, cff_Value *val) {
    ...
    while (1) {
        ...
        if (a != 15) { str_len += nibble_attr[a]; }
        else { break; }

        if (b != 15) { str_len += nibble_attr[b]; }
        else { break; }
